...
# Lazy import for pyzbar to avoid import errors when only generating QR codes
_pyzbar_decode = None
...
def _get_pyzbar_decode():
    ...
    if _pyzbar_decode is None:
        try:
            ...
            raise ImportError("pyzbar is required for QR code decoding. Please install zbar system library and pyzbar.")
...
def decode_qr_parts(
    ...
    for qr_img in qr_imgs:
        arr = np.array(qr_img)
        decoded_objs = _get_pyzbar_decode()(arr)
        if not decoded_objs:
